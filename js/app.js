...
function communeDominant(c){
  let best=null,bestValue=0;
  [...state.permitTypes].forEach(type=>{const v=permitTotal(c,type);if(v>bestValue){bestValue=v;best=type;}});
  return {type:best,value:bestValue};
}
function renderPermits(){
  ...
  const maxByType={};
  Object.keys(PERMIT_TYPES).forEach(type=>{maxByType[type]=Math.max(...state.sitadel.communes.map(c=>permitTotal(c,type)),1);});
  state.communes.eachLayer(layer=>{
    const name=layer.feature.properties.nom;
    const selected=name===state.communeFilter;
    const c=byName[name];
    const dominant=c?communeDominant(c):{type:null,value:0};
    const typeColor=dominant.type?PERMIT_TYPES[dominant.type].color:"#0063cb";
    const intensity=dominant.type?Math.min(1,dominant.value/maxByType[dominant.type]):0;
    layer.setStyle({
      color:selected?"#000091":"#8fa6c9",
      weight:selected?2.2:.6,
      fillColor:selected?"#000091":typeColor,
      fillOpacity:selected?.45:(dominant.value>0?.12+intensity*.58:.05),
    });
